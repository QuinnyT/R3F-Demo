...
const ControllableModel: React.FC<{isAutoMoving: boolean, setIsAutoMoving: (arg: boolean) => void, character: Character, isApproaching: string, setIsApproaching: (arg: string) => void, commitPosition: (position: THREE.Vector3) => void}> =({isAutoMoving, setIsAutoMoving, character, isApproaching, setIsApproaching, commitPosition}) => {
...
function updateCameraToFitBoundingBox(camera, controls, center, maxDistance, fitOffset = 1.5) {
  // const size = box.getSize(new THREE.Vector3()); // 获取边界框的尺寸
  // const center = box.getCenter(new THREE.Vector3()); // 获取边界框的中心

  // const maxSize = Math.max(size.x, size.y, size.z);
  // const fitHeightDistance = maxSize / (2 * Math.atan((Math.PI * camera.fov / 180) / 2));
  // const fitWidthDistance = fitHeightDistance / camera.aspect;
  // const distance = fitOffset * Math.max(fitHeightDistance, fitWidthDistance);
  const distance = maxDistance * fitOffset;

  const direction = new THREE.Vector3(0, 0, 1).applyQuaternion(camera.quaternion).normalize();
  const newPosition = direction.multiplyScalar(-distance);
  // camera.position.lerp(newPosition, 0.05);
  console.log("origin Position", camera.position)
...
function updateCameraByChangingFov(camera, cameraToSubjectVector) {
  const cameraToObjectVectors = characterInterActions.map((item) => {
    return new THREE.Vector3().subVectors(item.position, camera.position);
  })
  // const a = new THREE.Vector3().subVectors(characterModel.position, state.camera.position);
  // const cameraToSubjectVector = new THREE.Vector3(0, -4, -5);
  // 计算夹角
...
const Scene: React.FC = () => {
  ...
  const [isApproaching, setIsApproaching] = useState<string>("BoyWithAnima");
    ...
    <div ref={canvasRef}>
      <Canvas style={{ width: '100%', height: '100%' }} camera={{ fov: 75, position: [1, 2, 9] }} >
        ...
        <Suspense fallback={null}>
          ...
          <ControllableModel isAutoMoving={isAutoMoving} setIsAutoMoving={setIsAutoMoving} character={characters[0]} isApproaching={isApproaching} setIsApproaching={setIsApproaching} commitPosition={commitPosition}/>
