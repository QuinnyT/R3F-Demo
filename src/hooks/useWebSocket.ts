...
//   setChatActions: (chatActions: ChatAction[]) => void;
// }
// export const useChats = create<UseChatStore>((set) => ({
//   chatActions: [],
//   setChatActions: (chatActions) => set(() => ({ chatActions }))
// }))

// async function loadProto() {
//   try {
//       const root = await protobuf.load("../lib/TCProto.proto");
//       const ToGame = root.lookupType("ToGame");
//       const FromGame = root.lookupType("FromGame");
...
const useWebSocket = (url: string) => {
  ...
  // const { ToGame, FromGame } = loadProto();
  // const { setChatActions } = useChats();

  // useEffect(() => {
  //   const ws = new WebSocket(url);
  //   ws.onmessage = function(event) {
  //     const data = JSON.parse(event.data);
  //     setMoveToActions(data.moveToActions);
  //   };
  // }, [])

  // useEffect(() => {
  //   console.log("moveToDatas", moveToDatas);
  //   setMoveToActions(moveToDatas);
  // }, [moveToDatas])

//   function setPlayers() {

// 	const decodedMessage: ToGame;
// 	if (decodedMessage.create_player_reqs.length > 0) {
// 		const newPlayers = decodedMessage.create_player_reqs.map(item => ({
...
// 	if (decodedMessage.move_actions.length > 0) {
// 		decodedMessage.move_actions.map(item => {
// 			setMovement(item);
// 		});
// 	}

// 	if (decodedMessage.bubble_actions.length > 0) {
// 		decodedMessage.bubble_actions.map(item => {
// 			setBubble(item);
// 			setTimeout(() => {
// 				clearBubble(item);
// 			}, item.duration);
...
// 	if (decodedMessage.teleport_actions.length > 0) {
// 		decodedMessage.teleport_actions.map(item => {
// 			setTeleport(item);
// 		});
// 	}

// 	if (decodedMessage.animate_actions.length > 0) {
// 		decodedMessage.animate_actions.map(item => {
// 			setAnimation(item);
// 			const animationDuration = 3000; // 应根据实际动画时长进行设定
// 			if (item.animate_state && !item.is_repeat) {
// 				setTimeout(() => {
// 					stopAnimation(item);
// 				}, animationDuration);
// 			}
